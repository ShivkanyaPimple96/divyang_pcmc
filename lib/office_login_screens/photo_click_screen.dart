...
import 'package:http/io_client.dart'; // ✅ NEW IMPORT
...
class _PhotoClickScreenState extends State<PhotoClickScreen> {
  ...
  Future<bool> _submitPhotoToAPI(File imageFile) async {
    ...
    // ✅ Use the trusting client instead of the default http client
    ...
    try {
      ...
      var request = http.MultipartRequest(
        'POST',
        Uri.parse('https://lc.pcmcdivyang.com/api/aadhar/PostOffice'),
      );
      ...
      // ✅ Use client.send() instead of request.send()
      var response = await client.send(request).timeout(Duration(seconds: 240));
